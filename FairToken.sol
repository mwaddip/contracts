...
contract CRO20 is IERC20, Auth {
    ...
    string constant _name = "CROVID-19";
    string constant _symbol = "CROVID";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 1000000000000 * 10**4 * (10 ** _decimals);
    ...
    constructor () Auth(msg.sender) {
        router = IDEXRouter(0xdADaae6cDFE4FA3c35d54811087b3bC3Cd60F348); //Crodex router
    ...
    function setRouter(address _router, address _pair) external authorized {
        // This function allows changing the main DEX. This setting controls
        // into which liquidity pool the taxed tokens are sold for the
        // marketing wallet, as well as to which pool autoLiquidity adds to.
        address oldrouter = address(router);
        router = IDEXRouter(_router);
        pair = _pair;
        _allowances[address(this)][oldrouter] = uint256(0);
        _allowances[address(this)][address(router)] = uint256(-1);
    }
